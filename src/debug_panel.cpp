...
void iterateRecursive(CProfileIterator *profileIterator, int level = 0)
{
    ...
    //{
        ...
        for (i = 0; !profileIterator->Is_Done(); i++, profileIterator->Next())
        {
            ...
            if (ImGui::TreeNode(profileIterator->Get_Current_Name(), "%d -- [%d] %s (%.2f %%) :: %.3f ms / frame (%d calls)",
                                i,level, profileIterator->Get_Current_Name(), fraction,
...
void DebugPanel::Render()
{
    ...
    if (ImGui::Begin("Debug"))
    {
        if (ImGui::BeginTabBar("###DebugTabs"))
        {
                ...
                float accumulated_time = 0, parent_time = iterator->Is_Root()
                                                              ? CProfileManager::Get_Time_Since_Reset()
                                                              : iterator->Get_Current_Parent_Total_Time();
                int   i;
                int   frames_since_reset = CProfileManager::Get_Frame_Count_Since_Reset();

                if (ImGui::TreeNode(iterator->Get_Current_Parent_Name(),
