...
bool BrushModel::CreateFacesFromPlanes(int flags)
{
    int j;
    m_bValid      = true;
    const size_t nFaces = m_lstFaces.size();
    ...
    auto currentMaterial = MaterialAssetPtr();
...
{
    char tmp[32];
    // sprintf(tmp, 32, "%d", (int)index);
    sprintf(tmp, "%d", static_cast<int>(index));
    ...
    for (size_t f = firstFace; f < lastFace; f++)
    {
        ...
        const auto pBrushFace = m_pModel->AddFace(pFace->points);
...
{
    const auto pNewFace = new BrushFace(this, pts);
...
void BrushModel::RemoveFace(const size_t idx)
{
...
{
    const ColorRGBA color = PointerHash(pEntity);
